...
import { MissionEditor, CompanyOverviewEditor } from './about-us-cms-form';
import { useAboutAdmin, adminDataToFormSections } from '../../hooks/use-about-admin';

const ACCORDIONS = [
  { key: 'company_overview', title: 'Company Overview', Editor: CompanyOverviewEditor },
  { key: 'mission', title: 'Mission', Editor: MissionEditor },
];

function buildDefaultFormValues(sections) {
  const base = ABOUT_US_DEFAULTS.sections;
  const merged = {};
...
export function AboutUsCmsView() {
  const { hasApi, isLoading, isSaving, saveAllSections, publishAll, query } = useAboutAdmin();

  const methods = useForm({
    defaultValues: buildDefaultFormValues(ABOUT_US_DEFAULTS.sections),
  });
    ...
    return (
      <InstructorWorkspaceShell>
        <Alert severity="warning">
          Set <code>VITE_SERVER_URL</code> to your Laravel API to manage About Us content.
        </Alert>
      </InstructorWorkspaceShell>
    );
  }
        ...
        <Stack spacing={3} sx={{ pb: 12 }}>
          <Stack spacing={1}>
            ...
            <Typography variant="body2" color="text.secondary">
              Edit company overview, mission, and banner image. Changes apply to the public page at{' '}
              <strong>{paths.about}</strong>.
            </Typography>
          </Stack>
          ...
          {isLoading ? (
            ...
            <Stack spacing={2}>
              {ACCORDIONS.map(({ key, title, Editor }) => (
                <Accordion
                  key={key}
                  defaultExpanded={key === 'company_overview'}
                  disableGutters
                  elevation={0}
